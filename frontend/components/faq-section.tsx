...
export default function FAQSection() {
	...
	return (
		<section ref={sectionRef} id="faq" className="w-full py-12 xs:py-16 sm:py-20 lg:py-40 px-4 xs:px-5 sm:px-6">
			<motion.div style={{ y }} className="mx-auto max-w-6xl">
				<div className="grid lg:grid-cols-2 gap-8 xs:gap-10 lg:gap-10">
					{/* Left Column */}
					<motion.div
					...
					>
						<div className="flex gap-3 xs:gap-4 flex-col">
							<div>
								<Badge variant="outline" className="text-xs xs:text-sm">FAQ</Badge>
							</div>
							<div className="flex gap-2 flex-col">
								<h4 className="text-2xl xs:text-3xl md:text-4xl lg:text-5xl tracking-tighter max-w-xl text-left font-regular text-gray-900">
									Have Questions? <br />
									<span className="text-blue-600">We Have Answers</span>
								</h4>
								<p className="text-base xs:text-lg max-w-xl lg:max-w-lg leading-relaxed tracking-tight text-muted-foreground text-left text-gray-600">
									Everything you need to know about ConveneHub and how it improves end-to-end event operations.
								</p>
							</div>
							<div className="mt-2 xs:mt-0">
								<Button className="gap-3 xs:gap-4 text-sm xs:text-base" variant="outline" onClick={handleWhatsAppClick}>
						...
						<Accordion type="single" collapsible className="w-full">
							{faqs.map((faq, index) => (
								<AccordionItem key={index} value={`item-${index}`}>
									<AccordionTrigger className="text-left text-base xs:text-lg font-semibold text-gray-900 hover:text-blue-600">
										{faq.question}
									</AccordionTrigger>
									<AccordionContent className="text-sm xs:text-base text-gray-600 leading-relaxed">
										{faq.answer}
									</AccordionContent>
								</AccordionItem>
							))}
