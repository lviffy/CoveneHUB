...
const allSections = ['features', 'services', 'projects', 'testimonials', 'pricing', 'faq']
...
export const HeroHeader = () => {
    ...
    React.useEffect(() => {
        ...
        return () => unsubscribe()    }, [scrollYProgress])

    // Intersection Observer for active section detection
    React.useEffect(() => {
        const sectionElements = allSections.map(id => document.getElementById(id)).filter(Boolean)
        ...
        <header className="relative z-[999]">
            <motion.nav
                ...
                className="fixed top-0 w-full transform-gpu">
                  {/* Main Navigation */}                <div className={cn(
                    ...
                    'border rounded-xl',
                        ...
                        <div className="absolute inset-0 bg-gradient-to-b from-white/40 via-transparent to-transparent" />                        <motion.div
                            className={cn(
                            ...
                            <div className="hidden lg:block flex-1 px-6">
                                <ul className="flex items-center gap-2">
                                    {menuItems.map((item, index) => (
                                        <li key={index} className="relative">                                            <Link
                                                href={item.href}
                                                ...
                                                <motion.div
                                                    className="absolute inset-0 bg-gradient-to-r from-blue-100/80 via-blue-50 to-blue-100/80 border border-blue-300 rounded-lg"
                                                ...
                                                />
                                                  {/* Liquid shimmer effect - shows for both hover and active states */}
                                                <motion.div
                                                    className="absolute inset-0 bg-gradient-to-r from-transparent via-white/60 to-transparent"
                                                    initial={{ x: '-100%', opacity: 0 }}
                                                    animate={{
                                                        x: (hoveredItem === index && !isActiveSection(item.href)) || isActiveSection(item.href) ? '100%' : '-100%',
                                                        opacity: (hoveredItem === index && !isActiveSection(item.href)) || isActiveSection(item.href) ? 1 : 0
                                                    }}
                                                    transition={{ 
                                                        duration: 0.6, 
                                                        ease: "easeInOut",
                            ...
                            {/* Mobile Menu Button */}<motion.button
                                ...
                                whileTap={{ scale: 0.95 }}                                className={cn(
                                ...
                                <div className="relative w-4 h-4">
                                    <motion.div
                                        ...
                                        transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}                                        className="absolute inset-0">
                    ...
                    {menuState && (
                        <>                            {/* Backdrop */}
                            ...
                            <motion.div
                                ...
                                <div className="absolute inset-0 bg-gradient-to-br from-gray-50/50 via-transparent to-blue-50/30" />
                                ...
                                </div>                                {/* Mobile CTA Section */}
                                <div className="border-t border-gray-200 p-4 bg-gray-50/50">
                                    <ButtonGroup orientation="vertical" className="w-full flex-col gap-0">
                                        <motion.div whileTap={{ scale: 0.97 }} className="w-full">
                                            <Link href="/early-access" className="w-full block">
                                                <Button
                                                    variant="outline"
                                                    className={cn(
