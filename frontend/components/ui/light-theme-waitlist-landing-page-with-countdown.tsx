...
export function WaitlistExperience(): ReactElement {
  ...
  useEffect(() => {
    ...
    const colors = [
      new Color(0x195ADC), // ConveneHub Blue primary
      new Color(0x378FFA), // ConveneHub Blue tint 1
      ...
      <div className="relative z-10 min-h-screen">
        ...
        <div className="flex items-center justify-center min-h-screen px-4 pt-8">
          <div className="relative">
            <div className="relative backdrop-blur-xl bg-white/40 border border-[#C6BEF4]/60 rounded-3xl p-8 w-[420px] shadow-2xl">
              <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-[#F5F4FE]/80 to-transparent pointer-events-none" />

              <div className="relative z-10">
                {!isSubmitted ? (
                  <>
                    <div className="mb-8 text-center">
                      <h1 className="text-4xl font-bold text-[#010101] mb-4 tracking-tight">Join the waitlist</h1>
                      <p className="text-[#010101]/70 text-base leading-relaxed font-normal">
                    ...
                    <div className="flex items-center justify-center gap-3 mb-6">
                      <div className="flex -space-x-2">
                        <div className="w-8 h-8 rounded-full bg-[#195ADC] border-2 border-white flex items-center justify-center text-white text-xs font-semibold">
                          E
                        </div>
                        <div className="w-8 h-8 rounded-full bg-[#7F56D9] border-2 border-white flex items-center justify-center text-white text-xs font-semibold">
                          O
                        </div>
                        <div className="w-8 h-8 rounded-full bg-[#378FFA] border-2 border-white flex items-center justify-center text-white text-xs font-semibold">
                          N
                        </div>
                      </div>
                      <span className="text-[#010101]/70 text-sm font-medium">~ 20+ Creators already joined</span>
                    </div>

                    <div className="flex items-center justify-center gap-6 text-center">
                      <div>
                        <div className="text-2xl font-bold text-[#195ADC]">{timeLeft.days}</div>
                        <div className="text-xs text-[#010101]/60 uppercase tracking-wide font-medium">days</div>
                      </div>
                      <div className="text-[#C6BEF4]">|</div>
                      <div>
                        <div className="text-2xl font-bold text-[#195ADC]">{timeLeft.hours}</div>
                        <div className="text-xs text-[#010101]/60 uppercase tracking-wide font-medium">hours</div>
                      </div>
                      <div className="text-[#C6BEF4]">|</div>
                      <div>
                        <div className="text-2xl font-bold text-[#195ADC]">{timeLeft.minutes}</div>
                        <div className="text-xs text-[#010101]/60 uppercase tracking-wide font-medium">minutes</div>
                      </div>
                      <div className="text-[#C6BEF4]">|</div>
                      <div>
                        <div className="text-2xl font-bold text-[#195ADC]">{timeLeft.seconds}</div>
                        <div className="text-xs text-[#010101]/60 uppercase tracking-wide font-medium">seconds</div>
                      </div>
                    </div>
                  </>
                ) : (
                  <div className="text-center py-4">
                    <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-[#DAEDFF] to-[#BEE0FF] flex items-center justify-center border border-[#5DB0FD]">
                      <svg className="w-8 h-8 text-[#195ADC]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <h3 className="text-xl font-bold text-[#010101] mb-2">You're on the list!</h3>
                    <p className="text-[#010101]/70 text-sm font-normal">We'll notify you when we launch. Thanks for joining!</p>
                  </div>
                )}
              </div>
            ...
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#5DB0FD]/20 to-[#A998ED]/20 blur-xl scale-110 -z-10" />
          </div>
        </div>
      </div>
    </main>
