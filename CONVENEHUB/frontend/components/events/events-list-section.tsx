...
export default function EventsListSection() {
    ...
    try {
      ...
      if (fetchedEvents && fetchedEvents.length > 0) {
        ...
        const eventCities = fetchedEvents
          ...
          .filter(Boolean)
          .map((city: string) => city.trim()) // Normalize whitespace
  ...
  const fetchUserBookings = async (userId: string) => {
    try {
      ...
      if (!error && data) {
        const bookedEventIds = new Set(data.map((booking: any) => booking.event_id))
  ...
  if (isLoading) {
    return (
      <section id="events-list" className="relative py-16 xs:py-20 sm:py-24 px-4 xs:px-5 sm:px-6 bg-white">
        <div className="relative mx-auto max-w-7xl">
          <div className="flex items-center justify-center py-16 xs:py-20">
            <div className="text-center">
              <Spinner className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 text-[#195ADC] mx-auto mb-3 xs:mb-4" />
              <p className="text-sm xs:text-base text-gray-600">Loading events...</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="events-list" className="relative py-16 xs:py-20 sm:py-24 px-4 xs:px-5 sm:px-6 bg-white">
        <div className="relative mx-auto max-w-7xl">
          <div className="text-center py-16 xs:py-20">
            <Film className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 mx-auto mb-3 xs:mb-4 text-gray-400" />
            <h3 className="text-xl xs:text-2xl font-bold text-gray-900 mb-2">No Events Available</h3>
            <p className="text-sm xs:text-base text-gray-600">Check back soon for exciting upcoming events!</p>
          </div>
        </div>
      </section>
    )
    ...
    <section id="events-list" className="relative py-16 xs:py-20 sm:py-24 px-4 xs:px-5 sm:px-6 bg-white">
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mb-10 xs:mb-12 sm:mb-16">
          <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between mb-6 xs:mb-8 gap-2 xs:gap-4">
            <div>
              <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-1 xs:mb-2">
                Upcoming Events
              </h2>
          ...
          {hasFilters && (
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              {locationFilter && (
                <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-full">
          ...
          {/* Minimal City Filter */}
          <div className="flex items-center gap-1.5 xs:gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">{cities.map((city) => (
            <button
              key={city}
        ...
        <div className="space-y-4 xs:space-y-5 sm:space-y-6">
          {filteredEvents.map((event, index) => {
            ...
            return (
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 xs:gap-5 sm:gap-6">
                  ...
                  <div className="flex-1 space-y-3 xs:space-y-4">
                    ...
                    <div>
                      <div className="flex items-start gap-2 xs:gap-3 mb-2">
                        {event.event_image ? (
                          <div className="w-14 h-14 xs:w-16 xs:h-16 md:w-20 md:h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 relative">
                            <Image
                          ...
                          <div className="w-9 h-9 xs:w-10 xs:h-10 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                            <Film className="w-4 h-4 xs:w-5 xs:h-5 text-gray-600" />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <h3 className="text-lg xs:text-xl md:text-2xl font-bold text-gray-900 group-hover:text-gray-900 transition-colors truncate">
                              {event.title}
                            </h3>
                            <span className={cn(
                              "px-2 xs:px-3 py-0.5 xs:py-1 text-[10px] xs:text-xs font-medium rounded-full flex-shrink-0",
                              event.status === 'published' ? "bg-green-100 text-green-700" :
                                event.status === 'checkin_open' ? "bg-blue-100 text-blue-700" :
                                  event.status === 'in_progress' ? "bg-orange-100 text-orange-700" :
                                    event.status === 'ended' ? "bg-gray-100 text-gray-600" :
                                      "bg-gray-100 text-gray-600"
                            )}>
                              {event.status.replace('_', ' ').toUpperCase()}
                            </span>
                          </div>
                          <p className="text-gray-600 text-xs xs:text-sm line-clamp-2">
                            {event.description}
                          </p>
                        </div>
                      </div>
                    ...
                    {/* Details Grid */}
                    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 xs:gap-3 sm:gap-4 text-xs xs:text-sm pl-0 xs:pl-0 md:pl-13">
                      <div className="flex items-center gap-1.5 xs:gap-2 text-gray-600">
                        <Calendar className="w-3.5 h-3.5 xs:w-4 xs:h-4 flex-shrink-0" />
                        <span className="truncate">{eventDate.toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric'
                        })}</span>
                    ...
                    {/* Event Badges */}
                    <div className="flex flex-wrap gap-2 pl-0 xs:pl-0 md:pl-13">
                      {/* Limited Spots Badge */}
                      {event.remaining > 0 && event.remaining <= 10 && (
                        <span className="inline-flex items-center px-2.5 py-1 text-[10px] xs:text-xs font-medium bg-gray-900 text-white rounded-full">
                          Limited to {event.capacity} Visitors
                        </span>
                      )}

                      {/* Outdoor Scene Badge - based on venue name containing outdoor keywords */}
                      {(event.venue_name?.toLowerCase().includes('outdoor') ||
                        ...
                        event.venue_name?.toLowerCase().includes('park')) && (
                          <span className="inline-flex items-center px-2.5 py-1 text-[10px] xs:text-xs font-medium border border-gray-300 text-gray-700 rounded-full">
                            Outdoor Scene
                          </span>
                        )}
                      ...
                      {event.ticket_price === 0 && (
                        <span className="inline-flex items-center px-2.5 py-1 text-[10px] xs:text-xs font-medium bg-green-100 text-green-700 rounded-full">
                          Free Entry
                        </span>
                      )}

                      {/* Filling Fast Badge */}
                      {fillPercentage >= 70 && fillPercentage < 100 && (
                        <span className="inline-flex items-center px-2.5 py-1 text-[10px] xs:text-xs font-medium border border-orange-300 text-orange-700 rounded-full">
                          Filling Fast
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col xs:flex-row xs:items-center gap-3 xs:gap-4 sm:gap-6 pl-0 xs:pl-0 md:pl-13">
                      {/* Price Display */}
                      <div className="flex items-center gap-2">
                        <span className="text-xs xs:text-sm text-gray-500">Price:</span>
                      ...
                      <div className="flex-1 flex items-center gap-2 xs:gap-3">
                        <div className="flex-1 h-1 xs:h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                        ...
                        <span className="text-[10px] xs:text-xs font-medium text-gray-500 tabular-nums">
                ...
                {/* Availability Indicator - Subtle pulse */}
                {!isSoldOut && (
                  <div className="absolute top-4 right-4 xs:top-6 xs:right-6 sm:top-8 sm:right-8 flex items-center gap-1.5 xs:gap-2">
                    <div className="relative">
                      <Circle className="w-1.5 h-1.5 xs:w-2 xs:h-2 fill-green-500 text-green-500" />
                      <Circle className="w-1.5 h-1.5 xs:w-2 xs:h-2 fill-green-500 text-green-500 absolute inset-0 animate-ping opacity-75" />
                    </div>
                    <span className="text-[10px] xs:text-xs font-medium text-green-600 hidden xs:inline">Available</span>
                  </div>
                )}
              </motion.div>
            )
        ...
        {filteredEvents.length === 0 && (
          <div className="text-center py-16 xs:py-20">
            <div className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3 xs:mb-4">
              <Calendar className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 text-gray-400" />
            </div>
            <h3 className="text-lg xs:text-xl font-bold text-gray-900 mb-2">No events in {selectedCity}</h3>
            <p className="text-sm xs:text-base text-gray-600 mb-4 xs:mb-6">Try selecting a different city</p>
            <Button
