...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: event, error: eventError } = await supabase
      .from('events')
      .select('event_id, title, status, ticket_price, platform_commission_percentage, settlement_status')
    ...
    const platformCommission = grossRevenue.times(eventCommissionPercentage / 100);
    const netPayout = grossRevenue.minus(razorpayFees).minus(platformCommission);
    ...
    const { data: rawResult, error: settlementError } = await supabase.rpc(
      'mark_event_as_settled',
      {
        ...
        p_platform_commission: financials.platform_commission,
